@php($active = old('is_active', $unit->is_active ?? true))

<div class="grid gap-4 md:grid-cols-2">
    <div>
        <label class="dp-label" for="name">Unit Name</label>
        <input class="dp-input" id="name" name="name" value="{{ old('name', $unit->name) }}" required>
    </div>
    <div>
        <label class="dp-label" for="code">Code</label>
        <input class="dp-input" id="code" name="code" value="{{ old('code', $unit->code) }}" required>
    </div>
    <div class="md:col-span-2">
        <label class="dp-label" for="description">Description</label>
        <textarea class="dp-textarea" id="description" name="description" rows="3">{{ old('description', $unit->description) }}</textarea>
    </div>
</div>
